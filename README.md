...
A single message describing several distinct events (`"I have an interview at 2pm, after which I've got to take a shower at 3pm and then dinner at 8pm"` — or as plainly as `"an interview at 2pm and a birthday party at 7pm"`) is split into that many separate tasks, each with its own title and time — but only when every clause resolves with a clear time of its own; if any part is ambiguous (`"buy milk and eggs tomorrow at 5pm"` — only one time for both), the whole message falls back to becoming one task instead of guessing at a partial split.
...
| Recurrence cadences | 5 (`daily`, `weekly`, `monthly`, `yearly`, `weekday`) |
...
│   │   ├── config.py               Env var loading (HF_TOKEN, SMTP_*, APP_TIMEZONE) via python-dotenv
...
- Recurrence supports `daily` / `weekly` / `monthly` / `yearly` / `weekday` (Mon–Fri) cadences on a single task row (still no full RRULE support, no per-occurrence history, no arbitrary N-day/custom cadences) and there's no time zone handling — all dates and timestamps are naive local server time, consistently, end to end. Monthly and yearly recurrence advance by a real calendar month/year (via `dateutil.relativedelta`, jumping from the original due date each time) instead of a flat day-count step, so a task due on the 31st lands on the last day of each following month instead of drifting a little later every cycle. "Every weekday" skips Saturday/Sunday both for the very first occurrence and every one after — if the natural next date would land on a weekend, it's pushed to the following Monday.
...
- The Docker setup (`Dockerfile` + `docker-compose.yml`, multi-stage: Node build → Python runtime) can't be build-tested end-to-end in this environment (no Docker available here) — but every individual step has been verified by literally running it: the frontend build's `--outDir` override was run for real and confirmed it writes only to the override path (not `backend/app/static`, so a stale local build can't leak in); the backend half was verified by installing `requirements.txt` into a **fresh** virtualenv (not the dev one, which has extra packages) and, with only those packages, running the real startup sequence (`ensure_schema()`, constructing the FastAPI app, booting uvicorn, and hitting `/health` with the exact command `docker-compose.yml`'s healthcheck uses) — all of it passed. That surfaced one real bug this way: `zoneinfo` (used by `APP_TIMEZONE`) needs an IANA time zone database, which was only present via `tzdata` as a transitive, **Windows-only** dependency of `tzlocal` — meaning it likely would have silently failed on the Linux container the moment `APP_TIMEZONE` was actually set, the exact scenario that setting exists for. Fixed by depending on `tzdata` directly. What's still unverified is Docker itself — the build step, layer caching, and the named volume's actual behavior on a real Docker daemon — so run `docker compose up --build` yourself before relying on it in production. `.dockerignore` excludes `backend/app/static` and `.git`, so a locally-built frontend or your git history can't leak stale files into the image or bloat the build context.
