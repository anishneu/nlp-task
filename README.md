...
Every reply's base wording comes from a small pool of 3-4 hand-written phrasings per message type (`app/nlp/phrasing.py`), picked at random — so "remind me to water the plants tomorrow" might come back as "Got it — I've scheduled..." one time and "Done! ... is on the calendar" the next. This runs unconditionally, with zero added latency (it's local string selection, no network call), and is what keeps a chat with several messages from reading like the same three sentences on repeat.

On top of that, every reply can optionally also pass through a small LLM (`google/gemma-2-2b-it`) that rephrases whichever of those base phrasings got picked into something fully custom — the underlying facts (task titles, dates, links) are computed exactly as before and the LLM's only job is wording, so "Got it — I've scheduled X for Y" can come back as "Sounds good, I'll remind you about X on Y!" without risking the actual data. This one's **off by default** (`HF_REPLY_ENABLED=false`): unlike intent classification and task matching, which only make an HF call situationally, this one would run on nearly every message, and free-tier chat-completion latency (2-6s+ per call, sometimes worse) makes it the dominant cost of a response — the local phrase pool above gets you most of the "doesn't feel robotic" benefit for free, so this is worth turning on only if you want maximally varied wording and don't mind the latency. Set `HF_REPLY_ENABLED=true` to turn it on — falls back to the plain (but still varied) template instantly if the call fails or `HF_TOKEN` isn't set either way (see [Limitations](#limitations) for a real caveat about this one's free-tier quota).
...
| Conversational tone — phrasing | 3-4 hand-written wording variants per message type (`app/nlp/phrasing.py`), picked at random locally — always on, zero added latency | Implemented |
| Conversational tone — LLM rephrasing | Hugging Face chat-completion (`google/gemma-2-2b-it`, configurable via `HF_REPLY_MODEL`) rewords whichever local phrasing was picked, and generates conversation titles — gated by its own `HF_REPLY_ENABLED` flag (**off by default**, unlike the other HF features) since it runs on nearly every message rather than only situationally, making it the dominant source of response latency; plain (still varied) template / truncated title as automatic fallback either way | Implemented |
...
| Reply phrasing variants | 54 across 18 message types (3-4 each) — local, no HF call needed |
...
│   │   │   ├── phrasing.py         Local reply wording variants (3-4 per message type), picked at random — no HF call
...
- All HF-backed features need `HF_TOKEN`; without it, the app runs entirely on regex/template fallbacks. Reply rephrasing and title generation need `HF_REPLY_ENABLED=true` as well — they're off by default (see below).
- Reply rephrasing (and conversation title generation, gated by the same flag) is a live LLM call that runs on nearly every message rather than only situationally like the other two HF features — free-tier chat-completion latency (2-6s+ per call, sometimes worse) made it the dominant cost of a response, so `HF_REPLY_ENABLED` defaults to `false`. It also draws from a much smaller, shared third-party quota than intent classification/task matching (which run on HF's own free `hf-inference` tier) — it can run out from moderate use even when enabled. Falls back to the plain template reply when exhausted, with a circuit breaker to avoid repeated latency and a fallback model for non-quota failures. Note: the shared quota means pointing `HF_REPLY_MODEL` at a different provider does *not* grant a fresh quota — it's one pool across all of them.
